    ...
    : QMainWindow(parent)
{
    setupUi(this);
    contSplashScreen= 0;
    splash = new QSplashScreen(QPixmap("/usr/share/pixmaps/brisa/BrisaSplashScreenLightSwitcher.png"));
...
void LightSwitcher::getTargetResponse(QString response, QString method)
{
    if(method == "GetTarget") {
        if (response == QString("1")) {
            targetLabel->setPixmap(QPixmap(QString::fromUtf8("/usr/share/pixmaps/brisa/onlight1.png")));
        } else if (response == QString("0")) {
            targetLabel->setPixmap(QPixmap(QString::fromUtf8("/usr/share/pixmaps/brisa/offlight1.png")));
        } else {
...
void LightSwitcher::getStatusResponse(QString response, QString method)
{
    if(method == "GetStatus") {
        if (response == "1") {
            statusLabel->setPixmap(QPixmap(QString::fromUtf8("/usr/share/pixmaps/brisa/onlight1.png")));
        } else if (response == "0") {
            statusLabel->setPixmap(QPixmap(QString::fromUtf8("/usr/share/pixmaps/brisa/offlight1.png")));
        } else {
...
void LightSwitcher::processSplashScreen()
{
    ...
    case 1:
        splash->showMessage(QObject::tr("Setting up Brisa Lightswitcher..."),topLeft, Qt::blue);
        break;
    case 2:
        splash->showMessage(QObject::tr("Creating Brisa Lightswitcher..."),topLeft, Qt::blue);
        break;
    }
