...
def set_level(level, filename):
    ...
    if filename == 'stdout':
        ...
        filename = os.path.join(brisa_home, 'brisa.log')
        logging.filename = filename
...
# Set this manually because retrieving this from config was causing circular
# imports. When shipping, set it to INFO. For developing, just leave it DEBUG.
output = config.get_parameter('brisa', 'logging_output') or 'stdout'
level = config.get_parameter('brisa', 'logging') or 'DEBUG'
