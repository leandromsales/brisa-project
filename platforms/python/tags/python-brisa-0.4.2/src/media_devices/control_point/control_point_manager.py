...
from upnp.UPnPMessageHandler.device_description_object import \
...
class ControlPointManager(object):
    ...
    def searchDevices(self):
        ...
        for dev in self.control_point.ssdp_know:
            ...
            if (deviceType == 'urn:schemas-upnp-org:device:MediaServer:1'):
                ...
                control_url = self.get_service_control_url(device_services, \
                ...
                if (deviceType == 'urn:schemas-upnp-org:device:MediaRenderer:1'):
                    ...
                    device_services = deviceObject.get_services()
                    control_url = \
                        self.get_service_control_url(device_services,\
    ...
    def getSearchCapabilites(self, udn):
        ...
        print "Search Capabilities in " + device.get_friendly_name() + ": " +\
        device.contentDirectorySOAP.GetSearchCapabilities()

    def browse(self, id, udn, type="BrowseDirectChildren"):
        device = self.server_devices[udn]
        print "Browsing " + device.get_friendly_name() #+ ": "
        #browseResponse = cds_soap_proxy.Browse('0', 'BrowseMetadata',\
        # '*', '0', '0', '')
        browseResponse = device.SOAPService.Browse(id, type, '*', '0', \
                                                   '20', '')
    ...
    def get_media_url(self, server_udn, media_id):
        ...
        browseResponse = device.SOAPService.Browse(media_id, \
                                    "BrowseMetadata", '*', '0', '20', '')

        elt = DIDLElement.fromString(browseResponse['Result'])

        # first item (since it is a media item), first resource and its url
        #(resource.data = url)
        item = elt.getItems()[0]
