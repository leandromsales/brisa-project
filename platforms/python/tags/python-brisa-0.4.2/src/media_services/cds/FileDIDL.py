...
def getClassMT(name, mimetype = None, fp = None):
    '''Return a tuple of the DIDLLite class and mimetype responsible for the \
        named/mimetyped/fpd file.
    '''
    ...
    ty = mimetype.split('/')[0]
    if mimetoclass in has_key(mimetype): #Provavel erro troca do . por in
        klass = mimetoclass[mimetype]
    elif mimetoclass in has_key(ty):
...
def buildClassMT(baseklass, name, *args, **kwargs):
    ...
    class ret(baseklass, klass):
        pass
    ret.__name__ = '+'.join(map(lambda x: '%s.%s' % (x.__module__, \
x.__name__), (baseklass, klass)))
