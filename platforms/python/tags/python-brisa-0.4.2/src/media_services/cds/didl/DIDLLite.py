...
class Resource(object):
    ...
    def toString(self):
        return ElementTree.tostring(self.toElement())
    @classmethod
    ...
    def toElement(self):
        ...
        if self.date is not None:
            if isinstance(self.date, datetime):
                ElementTree.SubElement(root, 'dc:date').text = \
                self.date.isoformat()
            else:
    ...
    def fromElement(self, elt):
        ...
        for child in elt.getchildren():
            if child.tag.endswith('title'):
                ...
                res = Resource.fromString(ElementTree.tostring(child))
                self.res.append(res)
    @classmethod
...
self.storageMedium
        ...
        if self.publisher is not None:
            ElementTree.SubElement(root, 'dc:publisher').text = \
self.contributor
...
class MusicTrack(AudioItem):
    ...
    def toElement(self):
        ...
        if self.artist is not None:
            ElementTree.SubElement(root, 'upnp:artist').text = self.artist
        ...
        if self.playlist is not None:
            ElementTree.SubElement(root, 'upnp:playlist').text = self.playlist
...
self.createClass
        ...
        for i in self.searchClass:
            ElementTree.SubElement(root, 'upnp:searchclass').text = i
...
class DIDLElement(_ElementInterface):
    ...
    def fromString(cls, aString):
        ...
        for node in elt.getchildren():
            ...
            new_node = upnp_class.fromString(ElementTree.tostring(node))
