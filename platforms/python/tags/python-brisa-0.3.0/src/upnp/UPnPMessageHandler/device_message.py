...
class DeviceMessage(object):
    ...
    def _create_serviceList(self):
        ...
        if len(device_services) > 0:
            ...
            service_list_element = SubElement(self.deviceElement, "serviceList")
    ...
    def _create_embeddedDevices(self):
        ...
        if (self.device.is_rootDevice()):
           ...
           if len(embedded_devices) > 0:
               device_list_element = SubElement(self.deviceElement, "deviceList")
               for embedded_device in embedded_devices:
                   embedded_device_description = DeviceMessage(embedded_device)
                   device_list_element.append(embedded_device_description.create_description())
