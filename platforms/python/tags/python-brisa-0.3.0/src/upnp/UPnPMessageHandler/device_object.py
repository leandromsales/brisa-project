...
class DeviceObject(object):
    ...
    def _parse_device(self):
        self.device.set_complete_device_type_version(self.tree.findtext('.//{%s}deviceType' % self.ns))
        self.device.set_friendly_name(self.tree.findtext('.//{%s}friendlyName' % self.ns))
        self.device.set_manufacturer(self.tree.findtext('.//{%s}manufacturer' % self.ns))
        self.device.set_manufacturer_url(self.tree.findtext('.//{%s}manufacturerURL' % self.ns))
        self.device.set_model_description(self.tree.findtext('.//{%s}modelDescription' % self.ns))
        self.device.set_model_name(self.tree.findtext('.//{%s}modelName' % self.ns))
        self.device.set_model_number(self.tree.findtext('.//{%s}modelNumber' % self.ns))
        self.device.set_model_url(self.tree.findtext('.//{%s}modelURL' % self.ns))
        self.device.set_serial_number(self.tree.findtext('.//{%s}serialNumber' % self.ns))
        self.device.set_udn(self.tree.findtext('.//{%s}UDN' % self.ns))
        self.device.set_upc(self.tree.findtext('.//{%s}UPC' % self.ns))
        self.device.set_presentation_url(self.tree.findtext('.//{%s}presentationURL' % self.ns))

    def _parse_services(self):
        for xml_service_element in self.tree.findall('.//{%s}service' % self.ns):
            service_type     = xml_service_element.findtext('{%s}serviceType' % self.ns)
            service_id       = xml_service_element.findtext('{%s}serviceId' % self.ns)
            control_url      = xml_service_element.findtext('{%s}controlURL' % self.ns)
            eventSub_url     = xml_service_element.findtext('{%s}eventSubURL' % self.ns)
            presentation_url = xml_service_element.findtext('{%s}presentationURL' % self.ns)
