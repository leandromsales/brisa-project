#from pyinotify.pyinotify import WatchManager, Notifier, EventsCodes, ProcessEvent
...
plugin_classes = []
mask = EventsCodes.IN_DELETE | EventsCodes.IN_CREATE | EventsCodes.IN_MODIFY | EventsCodes.IN_MOVED_FROM | EventsCodes.IN_MOVED_TO
...
class ServerProcessEvent(ProcessEvent):
    ...
    def process_IN_CREATE(self, event):
        ...
        if os.path.isdir(path):
            ...
            logging.info(path + ' was added to shared directory and plugin ' \
                     + plugin.name + " handles the process of addition")
