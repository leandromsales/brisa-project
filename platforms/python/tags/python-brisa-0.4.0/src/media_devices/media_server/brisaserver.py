...
def parse_args():
    usage = "\nBrisa UPnP MediaServer version " + brisa_config.conf.brisa_version + " \nusage: python %prog [options] arg"
    ...
    if options.address is None:
        if brisa_config.contains("connection", "listenif"):
            ...
            try:
                ...
                print(
'Can not get an ip address give it using the option --address')
                logging.error(
...
def show_startup_message(listenURL):
    logging.info("Brisa UPnP Media Server v. " + brisa_config.conf.brisa_version)
    logging.info("Server is listening at address " + listenURL)
    logging.info("Press Ctrl+C to stop the server")
    print "Brisa Media Server version %s started. Please refer to %s for more information." % (brisa_config.conf.brisa_version, logging.filename)
...
def create_root_device(listenURL, serverName):
    ...
    root_device.set_manufacturer("Brisa Team. Embedded Laboratory and INdT Brazil")
    root_device.set_manufacturer_url("https://garage.maemo.org/projects/brisa/")
    root_device.set_model_description("An Opensource UPnP A/V Media Server")
    root_device.set_model_name("Brisa Media Server version " + brisa_config.conf.brisa_version)
    root_device.set_model_number(brisa_config.conf.brisa_version)
    root_device.set_model_url("https://garage.maemo.org/projects/brisa/");
    root_device.set_serial_number("0000004")
    return root_device
...
def main():
    ...
    serverName = "Brisa MediaServer at " + socket.gethost
