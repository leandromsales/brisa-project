...
def localItemExist(local_path, sizebytes=None):
    if sizebytes != None:
        count = Item.select(AND(Item.q.local_path == local_path, Item.q.sizebytes == sizebytes)).count()
    else:
        count = Item.select((Item.q.local_path) == local_path).count()
    return (count > 0)

def makeAllObjectsInvalid():
    object_valid = {'valid': '0'}
    Object(**object_valid)

#def removeAllInvalidItems():
#    from sqlobject.sqlbuilder import Delete
#    db.connection.query(db.connection.sqlrepr(Delete(Item.sqlmeta.table, Item.q.valid = '0')))

def sync_database():
    container = selectContainerByTitle("Audio Broadcast")[0]
    removeItemsAndContainerOfAContainer(container)

    container = selectContainerByTitle("Video Broadcast")[0]
    removeItemsAndContainerOfAContainer(container)

    container = selectContainerByTitle("Internet Photos")[0]
    removeItemsAndContainerOfAContainer(container)
