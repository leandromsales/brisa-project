...
def http_call(method, url, body='', headers={}):
    ...
    if body or headers:
        con.request(method, parsed_url[2], body=body, headers=headers)
    else:
        return None
...
def url_fetch(url, filename='', attempts=2, interval=0.1):
    ...
    for k in range(attempts):
        log.info('[%d] Fetching %r (attempt %d)' % (time(), url, k))
        req = urllib2.Request(url)
        try:
            handle = urllib2.urlopen(req)
        except IOError, e:
            if hasattr(e, 'reason'):
                log.warning('[%d] We failed to reach a server. Reason: %s' %
                            (time(), e.reason))
            elif hasattr(e, 'code'):
                log.warning('[%d] The server couldn\'t fulfill the request. \
                            Error code: %s' % (time(), e.code))
            handle = None
        finally:
            if handle != None:
                if not filename:
                    ...
                    log.debug('url_fetch: writing data to filename %s' %\
                              filename)
                    # Writing mode
