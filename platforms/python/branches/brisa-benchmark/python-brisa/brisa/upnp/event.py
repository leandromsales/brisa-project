...
from brisa.utils.network import http_call
...
class EventManager(CustomResource):
    ...
    def _subscribe(self, uri, request, response, compressed_headers):
        ...
        if request_status == 200:
            ...
            t_call = ThreadedCall(self._auto_remove_subscriber, None, None,
                 None, None, timeout * 1.1, "uuid:" + str(subscriber.subscription_id))
            t_call.start()
...
class Subscriber:
    ...
    def __init__(self, subscription_duration, delivery_url, http_version):
        ...
        thread_manager.register_stop_function(self.looping_call.stop)
    ...
    def stop(self):
        ...
        #HACK - Add an API to remove the stop function
        thread_manager._stop_functions.remove(self.looping_call.stop)
