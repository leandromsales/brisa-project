...
class SSDPServer(object):
    ...
    def datagram_received(self, data, (host, port)):
        ...
        if cmd[0] == 'M-SEARCH' and cmd[1] == '*':
            ...
            log.info('Received unknown SSDP command %s from %s:%s', cmd, host,
                     port)
    ...
    def _discovery_request(self, headers, (host, port)):
        ...
        [append('%s: %s' % (k, v)) for k, v in self.own[right_key].items()]
        response.extend(('', ''))
        ...
        log.debug('DiscoveryRequest response sent to (%s, %d)', host, port)
    ...
    def stop(self):
        ...
        self.renew_loop = self.udp_listener = self.udp_transport = None
        self._callbacks = {}
