...
class ControlPointAV(ControlPoint):
    ...
    def _set_current_server_device(self, device):
        """ This method sets the current server device.

        @param device: The device to be set.

        @type device: RootDevice class type
        """
    ...
    def _set_current_renderer_device(self, device):
        """ This method sets the current render device.

        @param device: The render device to be set.
    ...
    def _new_device_event_impl(self, cargo, device_object):
        """ Method which sets the services and namespaces to their respectives
            devices.

        @param cargo: Information about the device
        @param device_object: Instance of DeviceObject

        @type cargo: dict
        @type device_object: DeviceObject
        """
        # MediaServer and MediaRenderer
        if not device_object and self._ssdp_server:
            self._ssdp_server.discovered_device_failed(cargo)
            return

        url_info = parse_url(device_object.location)
        root_device = device_object.device

        log.info('New device found: %s' % root_device.udn)

        if root_device.udn not in [d.udn for d in self._known_devices]:
            self._known_devices.append(root_device)
            log.info('Device %s added to the list' % root_device.udn)
        else:
            log.info('Device %s already on the list' % root_device.udn)
    ...
    def browse(self, object_id, browse_flag, filter, starting_index,
               requested_count, sort_criteria="dc:title", cpId=""):
        """Make browses in Media Server.

        This method browser the media server with udn id. The browse_flag
        must be BrowseDirectChildren or BrowseMetadatai

        @param object_id: Object currently being browsed.
        @param browse_flag: Option to be used for browsing. For further\
        informations see\
        www.upnp.org/standardizeddcps/documents/ContentDirectory1.0.pdf
        @param filter: A filter to indicate which metadata properties\
        are to be returned.\
        @param starting_index: Enumerates children under the container\
        specified by object_id
        @param requested_count: Requested number of entries under the\
        object specified by object_id
        @param sort_criteria: The chosen sorting criteria to be used.
    ...
    def search(self, container_id, search_criteria, filter, starting_index,
        ...
        to begin searching.
        @param search_criteria: The search chosen criteria to be used.
        @param filter: A filter to indicate which metadata properties\
        are to be returned.\
        @param starting_index: Enumerates children under the container\
        ...
        @param sort_criteria: The chosen sorting criteria.
    ...
    def get_search_capabilites(self):
        ...
        @return: fields which the server support searching.
        @rtype: dict
        """
        try:
            ss = self.current_server.SOAPService
    ...
    def get_sort_capabilities(self):
        ...
        @return: fields which the server support sorting.
        @rtype: dict
        """
        try:
            ss = self.current_server.SOAPService
    ...
    def play(self, id):
        ...
        @param id: The id of the object to be played.
