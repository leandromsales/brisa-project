...
def get_service_control_url(service_dict, service_type):
    ...
    @param service_dict: dict of services
...
class ControlPoint(object):
    ...
    def _new_device_event_impl(self, device_info, device_object):
        ...
        @param device_info: informations about the device
        @param device_object: the device object itself

        @type device_info: dict
        @type device_object: Device
        """
    ...
    def _removed_device_event(self, device_info):
        ...
        if device_found in self._known_devices:
            self._known_devices.remove(device_found)
            log.info('Device %s removed from the list' % udn)
        else:
            log.info('Device %s has been already removed from the list' % udn)
