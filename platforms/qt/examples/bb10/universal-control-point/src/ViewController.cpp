...
ViewController::ViewController(QObject *parent) :
	...
	connect(m_controlPoint, SIGNAL(deviceFound(brisa::upnp::controlpoint::Device*)), this,
	...
	connect(m_controlPoint, SIGNAL(multicastReceivedRaw(OutArgument)),
			this, SLOT(multicastEventRawReceived(OutArgument)));
}
...
void ViewController::multicastEventRawReceived(OutArgument raw) {
	...
	case DeviceList::DEVICE: {
		...
		foreach (Service *tempService, device->getServiceList())
		{
			...
			connect(subscription,
					SIGNAL(eventNotification(brisa::upnp::controlpoint::EventProxy *,QMap<QString, QString>)),
					this,
					SLOT(unicastEventReceived(brisa::upnp::controlpoint::EventProxy *,QMap<QString, QString>)));
