...
void ActionController::serviceCall(OutArgument arguments, QString method) {
	disconnect(m_currentService, SIGNAL(requestFinished(OutArgument, QString)),
	        this, SLOT(serviceCall(OutArgument,QString)));
	disconnect(m_currentService, SIGNAL(requestError(QString, QString)),
