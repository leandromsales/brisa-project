...
Flashlight::Flashlight(QObject * parent) :
	...
	const camera_error_t error = camera_open(CAMERA_UNIT_REAR,
			CAMERA_MODE_PREAD | CAMERA_MODE_PWRITE, &m_camera);
	if (CAMERA_EOK != error) {
...
void Flashlight::setEnabled(bool enabled) {
	...
	const camera_videolightmode_t mode = (
			m_enabled ? CAMERA_VIDEOLIGHT_ON : CAMERA_VIDEOLIGHT_OFF);
