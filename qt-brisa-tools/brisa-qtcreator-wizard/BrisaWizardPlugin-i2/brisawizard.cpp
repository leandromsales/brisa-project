...
static bool wasAccept = false;
 ...
 void BrisaWizard::accept()
 {
     ...
     QString varBrisaDevice = "d_" + firstCharClass.toLower() + lastCharsClass;
...
if(field("generateControlPointYES").toString() == "true"){
    ...
    BrisaStateVariable *v = serviceList[0]->getStateVariableList().at(0);
    block +=
            ...
            "\tQ_UNUSED(action);\n"
            "\tqDebug() << result[\""+ v->getAttribute(v->Name).toAscii() + "\"];\n"
            "}\n\n"
