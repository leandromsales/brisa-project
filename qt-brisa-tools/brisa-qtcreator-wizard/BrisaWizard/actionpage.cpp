...
void ActionPage::on_actionListWidget_DoubleClicked(){
    ...
    dialogArgument->clearStateVariables();
    foreach(BrisaArgument* argument, action->getArgumentList()){
        ...
        foreach(BrisaStateVariable *v, service->getStateVariableList())
            if(v->getAttribute(v->Name) == argument->getAttribute(argument->RelatedStateVariable))
                dialogArgument->addStateVariable(v);
    }
    ...
    if(dialogArgument->exec()){
       ...
       //dialog>clearStateVariableList();
        foreach(BrisaStateVariable *stateVariable, dialogArgument->getStateVariables()){
            bool found = false;
            foreach(BrisaStateVariable *v, service->getStateVariableList()){
                if(v->getAttribute(v->Name) == stateVariable->getAttribute(stateVariable->Name))
                    found = true;
            }
            if(!found)
