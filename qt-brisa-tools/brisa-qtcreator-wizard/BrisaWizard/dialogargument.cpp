...
void DialogArgument::updateTableWidget(){
    ...
    for(int i = 0; i < ui->tb_argument->rowCount(); i++){
        if(ui->tb_argument->item(i,0)->text().isEmpty()){
            qDebug() << i;
            ui->tb_argument->removeRow(i);
        }
    }

}

void DialogArgument::addStateVariable(BrisaStateVariable* v){
    this->stateVariableList.append(v);
}
...
void DialogArgument::clearStateVariables(){
    this->stateVariableList.clear();
}
...
{
    if((ui->tb_argument->selectedItems().count() >0) && (ui->tb_stateVariable->selectedItems().count() > 0)){
        QString argument = ui->tb_argument->itemAt(ui->tb_argument->currentRow(),0)->text();
        QString stateVariable = ui->tb_stateVariable->itemAt(ui->tb_stateVariable->currentRow(),0)->text();
       if((!argument.isEmpty()) && (!stateVariable.isEmpty())){
