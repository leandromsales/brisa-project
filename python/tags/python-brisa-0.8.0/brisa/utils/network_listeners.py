    ...
    def prepare_to_stop(self):
        ...
        if self.listening:
            ...
            try:
                ...
                log.debug("writing pipe of death: error")
                pass
            if self.close():
                self.cleanup()
                return True
...
class TCPListener(NetworkListener):
    ...
    def run(self):
        ...
        while self.listening:
            try:
                read, dummy1, dummy2 = select.select( \
                      [self.socket, self.pipe_of_death_reader], [], [])
                if (self.pipe_of_death_reader in read) or not self.listening:
                    break
                if not self.socket in read:
                    continue
                self.busy = True
    ...
    def run(self):
        ...
        while self.listening:
            try:
                read, dummy1, dummy2 = select.select( \
                     [self.socket, self.pipe_of_death_reader], [], [])
                if self.pipe_of_death_reader in read:
                    log.debug("select returned with signal to exit")
                    break
