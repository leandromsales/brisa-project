...
package_list = ["cherrypy", "sqlite3", "mutagen", "pyinotify", "uuid"]

for package in package_list:
    try:
        __import__(package)
    except ImportError:
        print "Please install %s before continue" % package
        sys.exit()


__version__ = brisa.__version__

#SVN=1
#if SVN:
#    import time
#    __version__ += "_SVN_"  + time.strftime('%Y_%m_%d')
