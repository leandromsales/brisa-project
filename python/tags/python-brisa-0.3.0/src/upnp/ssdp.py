...
class SSDPServer(DatagramProtocol):
    ...
    def registerRootDevice(self,rootDevice):
        ...
        if (rootDevice.is_rootDevice()):
            ...
            self.localRegister('%s::upnp:rootdevice' % deviceId, 'upnp:rootdevice', deviceLocation, deviceServer, 'max-age=1800')
            self.localRegister(deviceId, deviceId , deviceLocation, deviceServer, 'max-age=1800')
            self.localRegister(deviceId + "::" + deviceType, deviceType , deviceLocation, deviceServer, 'max-age=1800' )

            for service in rootDevice.get_services():
                serviceType = service.get_service_type_version()
                self.localRegister(deviceId + "::" + serviceType, serviceType, deviceLocation , deviceServer, 'max-age=1800' )
    ...
    def discoveryRequest(self, headers, (host, port)):
        ...
        if rightKey == 0:
            log.msg('Unknown Error in Discovery request for %s' % headers['st'])
            return
