...
class Service(object):
    ...
    def __init__(self, location, control_url,
        ...
        self.handle_urls(control_url, event_sub_url, presentation_url, scpd_url)
        self.handle_url_base()

    def handle_urls(self, control_url, event_sub_url, presentation_url, scpd_url):
        if not control_url.startswith('/') and not control_url.startswith('http'):
            self.control_url = "/%s" % control_url
        if not event_sub_url.startswith('/') and not control_url.startswith('http'):
            self.event_sub_url = "/%s" % event_sub_url
        if presentation_url and not presentation_url.startswith('/') and not control_url.startswith('http'):
    ...
    def get_service_type_version(self):
        return self.service_type_version

    def set_service_type_version(self, service_type_version):
        self.service_type_version = UPnPDefaults.SERVICE_TYPE_VERSION_PREFIX + service_type_version

    def set_complete_service_type_version(self, service_type_version):
        self.service_type_version = service_type_version
    ...
    def set_complete_service_id(self, service_id):
    ...
    def startService(self):
        ...
        serviceResource.putChild("scpd.xml", static.File(self.create_description_xml()))
