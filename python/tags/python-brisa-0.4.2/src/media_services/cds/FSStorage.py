...
class DynamTransfer(protocol.ProcessProtocol):
    ...
    pauseProducing = lambda x: \
                        x.proc.pauseProducing()
    resumeProducing = lambda x: \
                        x.proc.resumeProducing()
    ...
    def render(self):
        ...
        args = ['ffmpeg', '-i', path, '-b', '8000',
            #'-sc_threshold', '500000', '-b_strategy', '1', '-max_b_frames',
            # '6',
...
class FSItem(FSObject, Item):
    ...
    def doUpdate(self):
        ...
        self.res = [self.res]
        self.res.append(Resource(self.url + '/mpeg2', 'http-get:*:%s:*' % \
                                'video/mpeg'))
        self.res.append(Resource(self.url + '/xvid', 'http-get:*:%s:*' % \
                                'video/avi'))
        Item.doUpdate(self)
