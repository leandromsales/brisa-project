...
API_KEY = 'f544dbc725689997313a6437e1068b5a'
SECRET = '3281387735c48555'
...
def _get(method, auth=False, **params):
    ...
    '''
    logging.debug('brisa_flickr>> Calling method: %s, with parameters %%s'%\
                   method, params)
    ...
    logging.debug('brisa_flickr>> Openning url: %s' % url)

    response = urlopen(url)
    response_element = ElementTree.parse(response).getroot()
    response.close()
    logging.debug('brisa_flickr>> Got response: \n%s' %\
                  ElementTree.tostring(response_element))
...
def get_interesting_photos():
    '''
    Get a list of dictionaries containing
    interresting photos from flickr.
    Returns:
    photos A list of dictionaries in the form {'url': url of the photo,
                                               'name': name of the photo}
    '''
    photos_et = interestingness_getList()
    photos = []
...
def get_photos_urls(photos_et, size=None):
    ...
    photos = []
    for photo_et in photos_et.find('photos'):
        photos.append(get_photo_url(photo_et, size))
    return photos
...
def call_browser(url, browser='firefox'):
...
#    username='brisa_project'
    ...
    print get_interesting_photos()

#    NSID = people_findByUsername(username).find('user').get('nsid')
#    print photos_search(auth=True,user_id=NSID)

#    print get_user_photos(username)
#    print get_user_photos(username,private=True)
#
#    NSID = people_findByUsername(username).find('user').get('nsid')
#    print NSID
#    print people_getPublicPhotos(NSID)
#
#    NSID = people_findByUsername(username).find('user').find('username').text
#    print NSID
#
#    print people_findByUsername(username).find('user').get('nsid')
#
#
#    #flickr.interestingness.getList
#    print interestingness_getList()