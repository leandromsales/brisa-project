...
dbname = config.get_parameter('maemo_multimedia', 'dbname')
metastorage_file = config.get_parameter('maemo_multimedia', 'connection')
db_uri = path.join(config.brisa_home, dbname)
log.debug('db uri is %s' % str(db_uri))
log = log.getLogger('maemo Multimedia')


queue_manager = SQLQueueManager()
worker_thread = SQLWorkerThread(db_uri)
...
def create_database():
    shutil.copyfile(metastorage_file, db_uri)
    ...
    for row in k:
        ...
        sql = 'UPDATE metadata set id = %s WHERE filename = "%s"' % (index,
                                                                filename)
        queue_manager.put_and_join(sql)
        index += 1
...
def select_item(id):
    queue_manager.put_and_join('SELECT MIME FROM metadata WHERE id = "%s"' %
                       (unicode(id)))
...
class MetadataItemsList(object):
    ...
    def url_item(self, id):
        queue_manager.put_and_join('SELECT * FROM metadata WHERE id = "%s"' %
                           (unicode(id)))
        k = queue_manager.task_result
