...
class ControlGUI(object):
    ...
    def server_rescan(self, widget):
        ...
        rescan_method = proxy.get_dbus_method('rescan_folders', MSERVER_IF)
        try:
            rescan_method()
        except Exception, e:
    ...
    def server_reload(self, widget):
        proxy = self.get_server_proxy()
        reload = proxy.get_dbus_method('reload_config', MSERVER_IF)
        try:
            reload()
        except Exception, e:
    ...
    def stop_server(self):
        proxy = self.get_server_proxy()
        stop_method = proxy.get_dbus_method('halt', MSERVER_IF)
        try:
            stop_method()
        except Exception, e:
    ...
    def stop_renderer(self):
        ...
        stop_method = proxy.get_dbus_method('halt', MRENDERER_IF)
        try:
            stop_method()
        except Exception, e:
    ...
    def get_server_info(self):
        proxy = self.get_server_proxy()
        info_method = proxy.get_dbus_method('get_server_info', MSERVER_IF)
        try:
            return info_method()
        except Exception, e:
    ...
    def get_renderer_info(self):
        proxy = self.get_renderer_proxy()
        info_method = proxy.get_dbus_method('get_renderer_info', MRENDERER_IF)
        try:
            return info_method()
        except Exception, e:
...
def main():
    g = ControlGUI()
    thread_manager.register_stop_function(g.quit)
