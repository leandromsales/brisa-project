...
from brisa.services.web_server import CustomResource, WebServer, static
from brisa.config import manager
...
class MediaLibraryResource(CustomResource):
    """ Resource for serving Media Library files.
    """

    def getChildWithDefault(self, uri, request):
        id = uri.split('/')[-1].split('-')[1]
        path = facade.get_file_path(id)

        if path:
            return static.StaticFile(path)
        else:
            return static.Data('', '')


class MediaLibrary(Plugin):
    ...
    def load(self):
        ...
        WebServer().add_resource(self.name, MediaLibraryResource())
